...
if __name__ == "__main__":
    ...
    # 2. Load Real Data atau Fallback ke Dummy Data
    data_path = os.path.join(root_dir, 'data', 'processed', 'lampung', 'version_2')
    print(f"⏳ Loading 1 Batch from: {data_path}")
    ...
    try:
        ...
        # Ekstrak label dan mask dari dataset 
        # (Squeeze dimensi channel agar dari (B, 1, H, W) menjadi (B, H, W) untuk loss)
        ...
        labels = torch.randn(B, H, W).to(device)
        masks = torch.ones(B, H, W).to(device)
        positions = torch.tensor([15, 45, 75, 105, 135, 165, 195, 225, 255, 285, 315, 345]).float().to(device)

    # 3. FIX POSITIONS SHAPE (Mencegah error [12] vs [2, 12])
    if positions.dim() == 1:
    ...
    try:
        ...
        output_hetero = model_hetero(images, batch_positions=positions)
        ...
        print(f"❌ Error in Heteroscedastic Mode: {e}")
        import traceback
        traceback.print_exc()

    # ==========================================
    # TEST 2: EVIDENTIAL MODE
    # ==========================================
    ...
    try:
        ...
        output_evid = model_evid(images, batch_positions=positions)
        ...
        print(f"✓ Loss (Evidential): {loss_evid.item():.4f}\n")
    except Exception as e:
        print(f"❌ Error in Evidential Mode: {e}")
        import traceback
        traceback.print_exc()
